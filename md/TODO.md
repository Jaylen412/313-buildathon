...
**Status:** steps 2–6 are done — **demo milestone 2 works end to end**: the map, the drawer, and (behind the org token) the ranked owner-occupied households on hot block groups with a reason per score term, anonymized in demo mode. Next and last core step is 7, the outreach brief. Your open red items are D.1 (demo corridors — the data now offers candidates, see below) and section E (deadline, ownership).
